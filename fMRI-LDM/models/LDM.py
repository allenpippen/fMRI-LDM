...
class Resnet(torch.nn.Module):
    ...
    def forward(self, x, time):
        #x -> [1, 320, 64, 64] 表示一张特征图
        #time -> [1, 1280] 表示这份数据中噪音的比例
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, frames=1):
        ...
        self.frames = frames
        #in
        self.in_vae = torch.nn.Conv2d(12*self.frames*2, 320, kernel_size=3, padding=1)
        ...
        self.out = torch.nn.Sequential(
            ...
            torch.nn.SiLU(),
            torch.nn.Conv2d(320, 12*self.frames, kernel_size=3, padding=1),
        )
...
if __name__ == '__main__':
    ...
    out_vae = torch.randn(1, 12, 64, 64).to(device)
    pre_out_vae = torch.zeros(1, 12, 64, 64).to(device)
    out_vae = torch.cat([out_vae, pre_out_vae], dim=1)
    out_encoder = torch.randn(1, 77, 768).to(device)
    time = torch.LongTensor([26]).to(device)

    out = unet(out_vae=out_vae, out_encoder=out_encoder, time=time)
